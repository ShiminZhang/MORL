...
if __name__ == "__main__":
    ...
    log_file_path = "results_variant_c_svd.csv"
    with open(log_file_path, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            "update", "global_step", "weighted_reward",
            "vel", "surv", "ener",
    ...
    for update in range(1, num_updates + 1):
        ...
        for epoch in range(update_epochs):
            ...
            for start in range(0, num_steps, batch_size):
                ...
                for k in range(num_objectives):
                    ...
                    optimizer.zero_grad()
                     # IMPORTANT: compute "pure" per-objective gradient g_k
                    loss_k.backward(retain_graph=True)

                    grad_vectors[k] = get_actor_grad_vector(agent)
                ...
                if all(g is not None for g in grad_vectors.values()):
                    ...
                    # SVD
                    # U - [5894, 3]
                    U, S, V = torch.linalg.svd(G_norm, full_matrices=False)

                    # record S values
                    history_singular_values.append(S.detach().cpu().numpy())
                    ...
                    with torch.no_grad():
                        # U
                        z_mean = torch.mean(U, dim=0, keepdim=True)
                        z_std = torch.std(U, dim=0, keepdim=True) + 1e-8
                        z_standardized = (U - z_mean) / z_std
                        # m4
                        current_m4 = torch.mean(z_standardized ** 4, dim=0)
                    ...
                    # use Synthesizer (get non-linear weight alpha)

                    #  (A) Batch-averaged state/preference (NOT per-state mixing):
                    #     - alpha is computed from mb_obs_avg and mb_pref_avg, so the mixing is minibatch-level, not state-level.
                    #     - If conflicts are highly state-dependent (e.g., "about to fall" vs "stable walking"),
                    #       averaging may wash out critical signals and produce a "blurry" alpha.
                    alpha, synth_logits = synthesizer(mb_obs_avg, mb_pref_avg, S, V)

                    # alpha * U
                    g_task = torch.matmul(U, alpha)
                    ...
                    optimizer.zero_grad() # clean old calculations for grad_vectors

                    # get Critic gradients (backward)
                    v_loss = 0.5 * ((newvalue - b_returns[mb_inds]) ** 2).mean()
                    (vf_coef * v_loss).backward()
        ...
        if update % 10 == 0:
            ...
            print(f"Update {update} | V_Loss: {v_loss.item():.4f} | Weighted reward: {avg_weighted_reward:.2f}")
            print(f"  --> Raw Rewards: [Vel: {avg_raw_vel:.2f}, Surv: {avg_raw_sur:.2f}, Energy: {avg_raw_ene:.2f}]")
            print(f"  --> Alpha Sample: {alpha.detach().cpu().numpy()}")
            print(f"  --> Kurtosis m4: [Vel: {avg_m4[0]:.2f}, Surv: {avg_m4[1]:.2f}, Ener: {avg_m4[2]:.2f}]")
            if len(history_singular_values) > 0:
                # average S of most recent 10 update
                recent_S = np.array(history_singular_values[- (num_steps // batch_size * 10):])
                avg_S = np.mean(recent_S, axis=0)

                # Condition Number) s1/s3
                cond_num = avg_S[0] / (avg_S[2] + 1e-8)

                print(f"  --> Singular Values (S): [s1:{avg_S[0]:.4f}, s2:{avg_S[1]:.4f}, s3:{avg_S[2]:.4f}]")
                print(f"  --> Conflict Intensity (s1/s3): {cond_num:.2f}")
            #print(f"  --> Actor (PG) Loss: {pg_val:.4f} | Entropy: {ent_val:.4f}")
            print(f"  --> CosSim [Vel/Surv: {avg_cos_01:.3f}] [Vel/Ener: {avg_cos_02:.3f}] [Surv/Ener: {avg_cos_12:.3f}]")
