...
class PPOTrainerC(MORLTrainer):
    ...
    def _update(self, advantages, returns):
        ...
        b_vector_advantages = advantages.reshape((-1, self.num_objectives))
        ...
        for epoch in range(self.update_epochs):
            ...
            for start in range(0, self.num_steps, self.batch_size):
                ...
                # Variant C: Gradient mixing - compute loss for each objective separately
                total_pg_loss = 0

                for k in range(self.num_objectives):
                    # Get k-th objective advantage
                    adv_k = b_vector_advantages[mb_inds, k]

                    # Per-objective normalization
                    adv_k = (adv_k - adv_k.mean()) / (adv_k.std() + 1e-8)

                    # Compute PPO loss for k-th objective
                    loss1 = -adv_k * ratio
                    loss2 = -adv_k * torch.clamp(ratio, 1 - self.clip_coef, 1 + self.clip_coef)
                    loss_k_element = torch.max(loss1, loss2)

                    # Get weight for k-th objective
