"""
DM Control Humanoid environment wrapper for Multi-Objective Reinforcement Learning.
"""
...
class SteerableDMControlHumanoidWrapper(gym.Wrapper, BaseMORLEnv):
    """
    DM Control Humanoid wrapper with 2 objectives:
    1. Velocity (horizontal velocity - forward walking)
    ...
    def _compute_velocity_reward(self):
        ...
        # Humanoid uses center of mass velocity
        velocity = physics.center_of_mass_velocity()[0]  # x-direction
...
def make_dm_control_humanoid():
    """Helper function to create the wrapped dm_control humanoid environment."""
    dm_env = suite.load(domain_name="humanoid", task_name="walk")
    env = DmControlCompatibilityV0(dm_env)
    return SteerableDMControlHumanoidWrapper(env)
